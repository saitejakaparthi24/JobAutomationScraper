...
class IndeedScraper:
    ...
    @staticmethod
    def posted_within_last_week(relative_date_str):
        """
        Indeed uses “Posted X days ago” or “Just posted”.
        We map that into real dates.
        """
        today = datetime.now()

        if "Just posted" in relative_date_str or "Today" in relative_date_str:
            return True

        m = re.search(r"(\d+)\+?\s+day", relative_date_str)
        if m:
            days = int(m.group(1))
            return days <= 7
    ...
    @staticmethod
    def extract_external_apply(job_soup):
        """
        Extract external apply link if available.
        """
        # Indeed marks external employer apply buttons
        external = job_soup.select_one("a[data-apply-type='easily']")
        if external and external.get("href"):
            return "https://www.indeed.com" + external.get("href")

        external2 = job_soup.select_one("a[rel='nofollow']")
        if external2 and external2.get("href"):
            return external2["href"]
    ...
    def load_previous_ids(keyword):
        """
        Same logic as LinkedIn scraper — dedupe via yesterday's Excel.
        """
        ...
        try:
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):
                job_link = row[5]
                m = re.search(r"viewjob\?jk=(\w+)", job_link)
                if m:
    ...
    async def fetch_jobs_for_keyword(self, client, keyword):
        ...
        job_postings = []
        seen_today = set()
        seen_yesterday = self.load_previous_ids(keyword)

        print(f"\n🚀 Starting Indeed scrape for keyword: {keyword}")
        print(f"📌 Loaded {len(seen_yesterday)} dedupe IDs (from yesterday)")
        ...
        while True:
            ...
            print(f"🔄 Fetching Indeed page {page}...")
            ...
            try:
                ...
                for card in cards:
                    try:
                        ...
                        # Indeed job id
                        m = re.search(r"jk=(\w+)", job_href)
                        if not m:
                            continue
                        job_id = m.group(1)

                        if job_id in seen_today or job_id in seen_yesterday:
                            continue
                        seen_today.add(job_id)

                        job_link = "https://www.indeed.com" + job_href
                        ...
                        posted_text = posted.text.strip() if posted else ""

                        # Filter older postings
                        if not self.posted_within_last_week(posted_text):
                            continue

                        # Load job detail page to extract apply link
                        detail_resp = await client.get(job_link, headers=headers)
                        detail_soup = BeautifulSoup(detail_resp.text, "lxml")
                        external_apply = self.extract_external_apply(detail_soup)

                        job_postings.append({
                            "Title": title,
                            "Company": company,
                            "Location": location,
                            "Date Posted": posted_text,
                            "Keyword": keyword,
                            "Job Link": job_link,
                            "Mobile Link": external_apply or job_link  # required by DB
                        })
        ...
        print(f"✅ Completed Indeed for '{keyword}' — {len(job_postings)} jobs found.\n")
        return job_postings
...
async def main():
    ...
    runner_linkedin = ScraperRunner(db, linkedin_scraper, exporter, KEYWORDS)
    runner_indeed = ScraperRunner(db, indeed_scraper, exporter, KEYWORDS)

    async with httpx.AsyncClient(timeout=45.0) as client:
        print("\n============================")
        print("       LINKEDIN SCRAPER")
        print("============================")
        await runner_linkedin.run(client)

        print("\n============================")
        print("        INDEED SCRAPER")
        print("============================")
        await runner_indeed.run(client)
